...
export interface UnifiedPlaceOrderButtonHandle {
  handleOTPVerifySuccess: (response: any) => Promise<void>;
  handleOtpClose: () => void;
}

export const UnifiedPlaceOrderButton = forwardRef<
  UnifiedPlaceOrderButtonHandle,
    ...
    const sendOTP = async (): Promise<boolean> => {
      ...
      try {
        ...
        try {
          ...
          console.error("Send OTP parse error:", parseError);
        }

        if (!response.ok) {
          const errorMessage =
            (data && data.message) || rawText || "Failed to send OTP";
          Toast.show({
            type: "error",
            text1: "Error",
            text2: errorMessage,
        ...
        if (data && data.success) {
          onOtpRequested?.(guestDetails.phoneNumber, data.token);
          return true;
        }
        ...
        Toast.show({
          type: "error",
          text1: "Error",
          text2: (data && data.message) || rawText || "Failed to send OTP",
          text1Style: { fontSize: 16, fontWeight: "bold" },
          text2Style: { fontSize: 14, fontWeight: "bold" },
        });
        return false;
